...
menucoffre = {
    ...
    Events = {
        onSelected = function(self, _, btn, PMenu, menuData, result)
            ...
            ESX.TriggerServerCallback("Tikoz:InventaireAmmunation", function(inventory) 
                ...
                for i=1, #inventory.items, 1 do 
                    ...
                    if btn.name == item.name then
                        ...
                        TriggerServerEvent('Tikoz:CoffreDeposeAmmunation', item.name, tonumber(count))
                        OpenMenu("Menu :")
                    end
                end
            end)

            ESX.TriggerServerCallback("Tikoz:CoffreSocietyAmmunation", function(items)
               ...
               if btn.name == "Retiré" then
                ...
                for i=1, #itemstock, 1 do 
                    ...
                    if btn.name == itemstock[i].label then
                        ...
                        count = Keyboardput("Combien voulez vous déposé ? ", "", 15)
                        TriggerServerEvent('Tikoz:RetireCoffreAmmunation', itemstock[i].name, tonumber(count), itemLabel)
                        OpenMenu("Menu :")
                    end
